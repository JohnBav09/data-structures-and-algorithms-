...
const howMuchPencil = (str) => {
  ...
  let howMuchPencil = testarray.shift('') 
  return result;
};
...
const wordsToCharList = (arr) => {
  let wordsToCharList  = Array.concat(testArray, thirdArray, ['hi']);
...
listFoods.foreach([]).concat.splice
return result;
...
const splitFoods = (recipe) => {
  let result = [];
  let splitFoods = splitFoods.split(listfoods.Array);
  return result;
};
...
const removeEvenValues = (arr) => {
  let removeEvenValue = arr.splice(2, 1, 'never');
  console.log(removeEvenValue);
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  removeLastCharacters = numberOfCharacters.pop(str)
};
